...
fn scan_frontier(
    ...
    let jobs: Vec<(usize, usize)> = (0..frontier.len())
        .flat_map(|frontier_idx| (0..ctx.scan_images.len()).map(move |image_idx| (frontier_idx, image_idx)))
        .collect();
    ...
    let mut hits: Vec<(usize, Caller)> = jobs
        ...
        .flat_map_iter(|(frontier_idx, image_idx)| {
            ...
            callers.into_iter().map(move |caller| (*frontier_idx, caller))
        })
        .collect();
...
pub fn build_call_tree(
    ...
    for depth in 0..ctx.cfg.depth {
        if frontier.is_empty() || ctx.check_total() { break; }
        ...
        if !ctx.cfg.quiet {
            let msg = if n == 1 {
                format!("depth {}  ·  scanning {} files for 1 target",
                    depth, scan_total)
            } else {
                format!("depth {}  ·  scanning {} files for {} targets in parallel",
                    depth, scan_total, n)
            };
        ...
        let lane_count = ctx.cfg.workers.max(1).min(8);
        ...
        if !ctx.cfg.quiet {
            writeln!(w, "{}", c.dim(&format!(
                "  -> {} unique caller(s) found at depth {}", found_this_level, depth + 1
            ))).ok();
        }
    ...
    fn to_call_node(idx: usize, flat: &[FlatEntry]) -> CallNode {
        ...
        CallNode {
            ...
            callers:   e.children.iter().map(|&ci| to_call_node(ci, flat)).collect(),
        }
    }
