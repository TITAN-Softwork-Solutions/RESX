...
impl AsyncProgress {
    pub fn new(total: usize, lanes: usize, active: bool, color: bool) -> Self {
        ...
        let lanes = (0..lane_count).map(|_| AsyncLane {
    ...
    fn render(&self) {
        ...
        if self.color {
            eprint!("\r\x1b[2K  {}  {:>6}/{:<6}  \x1b[2mtotal jobs\x1b[0m\n", beam, done, self.total);
        } else {
            eprint!("\r\x1b[2K  {}  {:>6}/{:<6}  total jobs\n", beam, done, self.total);
        }
        ...
        for (idx, lane) in self.lanes.iter().enumerate() {
            ...
            if self.color {
                eprint!("\r\x1b[2K  {}  \x1b[2mworker {:>2}\x1b[0m  {:>6}  \x1b[2m{:<42}\x1b[0m\n",
                    lane_beam, idx + 1, lane_done, label);
            } else {
                eprint!("\r\x1b[2K  {}  worker {:>2}  {:>6}  {:<42}\n",
                    lane_beam, idx + 1, lane_done, label);
            }
        }
        let _ = std::io::stderr().flush();
    ...
    pub fn finish(&self) {
        ...
        let _g = self.render_lock.lock().unwrap();
        eprint!("\x1b[{}A\x1b[{}M", self.lanes.len() + 1, self.lanes.len() + 1);
        let _ = std::io::stderr().flush();
    }
}
...
impl ProgressBar {
    pub fn new(total: usize, active: bool, color: bool) -> Self {
        Self { total, active, color, done: AtomicUsize::new(0) }
    }
...
impl StageProgress {
    pub fn new(total: usize, active: bool, color: bool) -> Self {
        Self { total: total.max(1), done: 0, active, color }
    }
    ...
    pub fn tick(&mut self, label: &str) {
        ...
        let label = trunc_label(label, 38);
        eprint!("\r  {}  {:>2}/{:<2}  {:<38}", beam, self.done, self.total, label);
        let _ = std::io::stderr().flush();
    }
