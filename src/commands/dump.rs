...
use crate::disasm::{collect_api_calls, disassemble_at, find_string_refs, find_xrefs, ApiCall, Instruction};
use crate::edr::{check_prologue, EdrCheckResult};
use crate::intelli::{analyze_image, IntelliFinding};
use crate::output::{print_c_recomp, print_eat, print_iat, print_insns, print_pe_anomalies, print_sections, print_yara_matches, StageProgress};
...
pub fn run(
    ...
    let mut progress = StageProgress::new(count_dump_steps(cfg, only_metadata, want_recomp), !cfg.quiet && !cfg.json, c.on);
    ...
    let (target_rva, mut resolved_name, pdb_loaded) =
    ...
    let api_calls = if cfg.funcs_depth > 0 {
        ...
        if !cfg.json {
            print_api_calls(w, &calls, &insns, &resolved_name, c, &raw, &pe, &symbol_index, &exports, arch, image_base, cfg, target_rva);
        }
...
fn print_api_calls(
    ...
    print_calls_recursive(w, calls, insns, c, &root_image, cfg, 0, &mut visited, &mut dll_map, "  ");
}
    ...
    for (i, call) in calls.iter().enumerate() {
        ...
        if let Some(target) = syscall_target.as_ref() {
            ...
            if !target.classes.is_empty() {
                let classes = target.classes.iter().map(|v| format_class_value(*v)).collect::<Vec<_>>().join("|");
                writeln!(w, "{}{} {}", detail_prefix, c.dim("class :"), c.b_white(&classes)).ok();
            }
        }
        ...
        if can_recurse {
            if let Some(file_off) = image.pe.rva_to_offset(call.target_rva) {
                ...
                if let Ok(sub_insns) = disassemble_at(image.raw, file_off, call.target_rva, image.arch, image.image_base, image.exports, Some(image.symbol_index), &sub_cfg) {
                    let sub_calls = collect_api_calls(&sub_insns, image.pe, image.raw, image.symbol_index, image.image_base);
                    if !sub_calls.is_empty() {
                        let child_prefix = format!("{}{}   ", line_prefix, if is_last { " " } else { "│" });
                        print_calls_recursive(w, &sub_calls, &sub_insns, c, image, cfg, depth + 1, visited, dll_map, &child_prefix);
                    }
                }
            }
            ...
            if let Some(file_off) = target.image.pe.rva_to_offset(target.rva) {
                ...
                if let Ok(sub_insns) = disassemble_at(
                    ...
                    if !sub_calls.is_empty() {
                        ...
                        print_calls_recursive(w, &sub_calls, &sub_insns, c, &kernel_view, cfg, depth + 1, visited, dll_map, &child_prefix);
                    }
                }
            }
        }
...
fn resolve_syscall_trace_target(call: &ApiCall, insns: &[Instruction], cfg: &Config) -> Option<SyscallTraceTarget> {
    ...
    for kernel_name in ["ntoskrnl.exe", "ntkrnlmp.exe", "ntkrnlpa.exe", "ntkrpamp.exe"] {
        let Some(image) = load_trace_image(kernel_name, cfg) else { continue; };
...
fn resolve_kernel_symbol(image: &LoadedTraceImage, name: &str, cfg: &Config) -> Option<(u32, String)> {
    for candidate in kernel_name_candidates(name) {
        if let Some(export) = image.exports.iter().find(|e| e.name.eq_ignore_ascii_case(&candidate)) {
...
fn resolve_syscall_dispatch_target(
    ...
    let class_values = infer_immediate_arg_values(caller_insns, call.rva, Register::ECX, Register::RCX);
    ...
    for &class_value in &class_values {
        if let Some(target_rva) = resolve_dispatch_rva(kernel_image, &dispatcher, class_value) {
            ...
            if let Some((_, _, classes)) = resolved.iter_mut().find(|(rva, _, _)| *rva == target_rva) {
    ...
    Some(SyscallDispatchTarget { rva, symbol_name, classes })
}
...
fn extract_index_table_rva(instr: &iced_x86::Instruction) -> Option<u32> {
    if instr.mnemonic() != Mnemonic::Movzx || instr.op0_kind() != OpKind::Register || instr.op1_kind() != OpKind::Memory {
        return None;
    }
    if instr.op0_register() != Register::EAX || instr.memory_base() != Register::RCX || instr.memory_index() != Register::RAX {
        return None;
    }
...
fn extract_target_table_rva(instr: &iced_x86::Instruction) -> Option<u32> {
    if instr.mnemonic() != Mnemonic::Mov || instr.op0_kind() != OpKind::Register || instr.op1_kind() != OpKind::Memory {
        return None;
    }
    if instr.memory_base() != Register::RCX || instr.memory_index() != Register::RAX || instr.memory_index_scale() != 4 {
        return None;
    }
    Some(instr.memory_displacement64() as u32)
}

fn resolve_dispatch_rva(image: &LoadedTraceImage, dispatcher: &QsiDispatcher, class_value: u32) -> Option<u32> {
    let adjusted = class_value.checked_sub(dispatcher.class_bias)?;
    if adjusted > dispatcher.max_index {
        return None;
    }

    let index_off = image.pe.rva_to_offset(dispatcher.index_table_rva.checked_add(adjusted)?)?;
    let slot_index = *image.raw.get(index_off)? as u32;
    let target_slot_rva = dispatcher.target_table_rva.checked_add(slot_index.checked_mul(4)?)?;
...
fn infer_immediate_arg_values(insns: &[Instruction], call_rva: u32, arg32: Register, arg64: Register) -> Vec<u32> {
    let Some(pos) = insns.iter().position(|insn| insn.rva == call_rva) else { return Vec::new(); };
    ...
    for insn in insns[..pos].iter().rev().take(16) {
        match insn.iced.mnemonic() {
            ...
            Mnemonic::Jne | Mnemonic::Je | Mnemonic::Ja | Mnemonic::Jae | Mnemonic::Jb | Mnemonic::Jbe => {}
            _ => {}
        }
    }
...
fn best_symbol_name_for_rva(symbol_index: &SymbolIndex, image_base: u64, rva: u32) -> Option<String> {
