export interface CfgRenderDeps {
    cfgMnemonicClass(mnem: string): string;
    navigateRva(rva: string, label: string): void;
    normalizeRva(rva: string): string | null;
}
...
export function renderCfgVisual(cfgText: string, panel: HTMLElement, deps: CfgRenderDeps): void {
    ...
    blocks.forEach(blk => blk.edges.forEach(e => {
        if (e.target && blocks.has(e.target)) indeg.set(e.target, (indeg.get(e.target) || 0) + 1);
    }));
    ...
    while (q.length) {
        ...
        for (const edge of blk.edges) {
            if (!edge.target || !blocks.has(edge.target)) continue;
    ...
    blockIds.forEach(id => {
        if (!layerOf.has(id)) layerOf.set(id, (layerOf.get(start) || 0) + 1);
    });
    ...
    const incomingLanes = new Map<string, number>();
    ...
    const kindOf = (id: string, idx: number): string => {
        if (idx === 0 || id === entryId) return 'entry';
        const blk = blocks.get(id);
        if (!blk) return 'normal';
        if (blk.edges.some(e => e.kind === 'exit')) return 'exit';
        if (blk.edges.some(e => e.kind === 'taken') && blk.edges.some(e => e.kind === 'fallthrough')) return 'branch';
        if (blk.edges.some(e => e.kind === 'jump')) return 'jump';
        return 'normal';
    };

    let idx = 0;
    for (const [id, blk] of blocks) {
        if (blk.x == null || blk.y == null || blk.w == null || blk.h == null) continue;
    ...
    function focusEntryBlock(): void {
        ...
        const targetScale = Math.min(
            Math.max(0.55, Math.min((w - 80) / (entry.w + 140), (h - 80) / (Math.min(totalH, entry.h + 240)))),
            1
        );
        scale = targetScale;
    ...
    if (blockCount <= 18) fitToView();
    else focusEntryBlock();
}

function parseCfgText(text: string, deps: Pick<CfgRenderDeps, 'normalizeRva'>): { blocks: Map<string, CfgBlock>; entryId: string } {
    ...
    for (const line of text.split('\n')) {
        ...
        if (inEdges) {
            ...
            const txt = (im[3] || '').trim();
            const cmt = (im[4] || '').trim();
            if (!current.startRva) current.startRva = rva;
