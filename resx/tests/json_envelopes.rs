...
fn fixture(name: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("..").join("test").join(name)
}
...
fn peinfo_dump_and_metadata_commands_use_versioned_json() {
    ...
    let peinfo = run_json(&["peinfo", j58.to_str().unwrap(), "--json", "--no-color", "--quiet"]);
    assert_eq!(peinfo["schema_version"], 1);
    assert_eq!(peinfo["peinfo"]["file_name"], "J58.dll");
    assert!(peinfo["peinfo"]["sections"].as_array().is_some_and(|v| !v.is_empty()));

    let exports = run_json(&["eat", j58.to_str().unwrap(), "--json", "--no-color", "--quiet"]);
    ...
    let dump = run_json(&["dump", j58.to_str().unwrap(), first_export, "--json", "--no-color", "--quiet"]);
    assert_eq!(dump["schema_version"], 1);
    assert_eq!(dump["dump"]["function"], first_export);
    assert!(dump["dump"]["instructions"].as_array().is_some_and(|v| !v.is_empty()));

    let imports = run_json(&["iat", j58.to_str().unwrap(), "--json", "--no-color", "--quiet"]);
...
fn explain_command_uses_versioned_json() {
    let explain = run_json(&["explain", "NtQuerySystemInformation", "--json", "--no-color", "--quiet"]);
